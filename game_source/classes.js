...
const TIME_INTERVALS = [
  //24*60*60*1000, I swear no one will play this a whole day...
  60*60*1000,
  60*1000,
  1000
];
...
class Char {
  /*
  Represents char, which gamer will see on the screen, and which can be
  typed, an then faded out.
  */
  constructor(text, coordX = 0, coordY = 0, width = 30, height = 50) {
    ...
    this.status = "active"; // could be: 'active' | 'typed' | 'fallen'
  }
  createDOMNode() {
  ...
  newFrame(speed) {
    /*
    Implements rendering of chars movements' frame.
    */
    switch (this.status) {
      case "active":
  ...
  fellDown(field) {
    /*
    Controls if the char have reached an edge of the game field.
    */
  ...
  static getRandom() {
    /*
    Generates instances of 'Char' class,
    taking a random character from the charset.
    */
...
class Field {
  /*
  Represents game field, when all game process goes.
  Is resposible for showing and removing inner dom elements,
  showing inial game screen, raw game field, score screen.
  */
  ...
  setInitScreen(mainField) {
    let explainText = `Try to type all chars which you will see
    on the screen until they will fall down the game field.`;
...
class Game {
  /*
  Controls all game process, as rendering game frames, counting scores, lifes
  timing, handling chars movement, incrementing speed during a game.
  */
  constructor(minSpeed = Game.minSpeed/*px/sec*/, lifes = 3) {
    ...
    this.speed = minSpeed;
    this.gameStep = 0; // new game step increments speed and adds a new char
    this.nextCharTime = 0;
    this.presentChars = [];

    this.sounds = renderAudios(Game.SOUND_LINKS); // action sounds
  }
  updateTable(table) {
  ...
  beginGame(field) {
    return new Promise(resolve => {
      ...
      const difficultyChange = event => {
        // controls users' changes of game speed
        this.speed = +event.target.value;
        this.step = (Game.stepRelativeSpeed * 100) / this.speed;
      }
      const handleButton = () => {
        // on 'play' click removes initial screen
  ...
  nextStep(initSpeed, initStep) {
    /*
    Add new character on the screen and increments game speed,
    proportional to the initial game speed.
    */
    this.gameStep++;
    ...
    // There is game speed limit, configured in the static properties.
  ...
  charHandler(char) {
    /*
    If a character was touched or fallen, give fading animations' frame,
    and erase charater if it's done, else moves a character down.
    Also changes scores and lifes quantity based on action, happened with char.
    */
  ...
  trackKeys(event) {
    /*
    Event listener, on keydown, makes some charater on the screen typed.
    */
